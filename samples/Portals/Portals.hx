...
class Portals extends Template {
	...
	override function init() {
		...
		portalC.target = portalA;
		portalA.target = portalC;
		portalB.target = portalE;
		portalE.target = portalB;
		portalD.target = portalF;
		portalF.target = portalD;
...
typedef Portal = {
	target:Portal,
	// where entering this portal takes you
	collide:Array<Shape>,
	// shapes to always collide with belonging to portal when passing through
	ignore:Array<Shape>,

	// shapes to never collide with belonging to portal whe passing through
	body:Body,
	sensor:Shape,
	position:Vec2,
	// position of portal centre (local to body)
	direction:Vec2,
	// direction of portal (local to body)
	width:Float
	// width of portal
};
...
class PortalManager {
	...
	public function new(space:Space) {
		...
		space.listeners.add(new InteractionListener(CbEvent.BEGIN, InteractionType.SENSOR, PORTAL, PORTABLE, portalBegin));

		space.listeners.add(new InteractionListener(CbEvent.END, InteractionType.SENSOR, PORTAL, PORTABLE, portalEnd));

		space.listeners.add(new PreListener(InteractionType.COLLISION, PARTIAL, CbType.ANY_SHAPE, backCollision));
	}
	...
	function portalBegin(cb:InteractionCallback) {
		...
		if (bodyData.__portal_pairs != null) {
			for (pair in bodyData.__portal_pairs) {
				if ((pair.portalA == portal || pair.portalB == portal) && (pair.bodyA == body || pair.bodyB == body)) {
					portalPair = pair;
					break;
				}
			}
		...
		if (portalPair == null) {
			...
			var pcon = new PortalConstraint(portal.body, portal.position, portal.direction, targetPortal.body, targetPortal.position, targetPortal.direction,
				scale, body, clone);
...
class PortalConstraint extends UserConstraint {
	...
	public function new(portalBody1:Body, position1:Vec2, direction1:Vec2, portalBody2:Body, position2:Vec2, direction2:Vec2, scale:Float, body1:Body,
			body2:Body) {
