...
class PerlinSquares extends Template {
	...
	override function preStep(deltaTime:Float) {
		...
		var polygons = MarchingSquares.run(iso, bounds, cellSize, 2, gridSize, true, output);
...
class Perlin3D {
	public static inline function noise(x:Float, y:Float, z:Float) {
		...
		// untyped __int__ performs better than Std.int when we're only
		// targetting flash.
		var X:Int = untyped __int__(x);
		x -= X;
		X &= 0xff;
		var Y:Int = untyped __int__(y);
		y -= Y;
		Y &= 0xff;
		var Z:Int = untyped __int__(z);
		z -= Z;
		Z &= 0xff;
		#else
	...
	static inline function fade(t:Float)
		return t * t * t * (t * (t * 6 - 15) + 10);

	static inline function lerp(t:Float, a:Float, b:Float)
		return a + t * (b - a);
	...
	static inline function p(i:Int)
		return perm[i];
