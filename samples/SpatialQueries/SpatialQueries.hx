...
class SpatialQueries extends Template {
	...
	override function init() {
		...
		cannonGunFilter = new InteractionFilter(/*collisionGroup*/ 2, /*collisionMask*/ ~2);
	...
	override function postUpdate(_:Float) {
		...
		for (i in 0...20) {
			...
			var result = space.convexCast(cannonBallShape, /*deltaTime*/ 1, /*liveSweep*/ false, cannonGunFilter);
			...
			if (result != null) {
				...
				debug.drawLine(result.position, result.position.addMul(result.normal, 15, true), 0xaa00);
		...
		for (i in 0...20) {
			...
			var result = space.rayCast(ray, /*inner*/ false, cannonGunFilter);
			...
			if (result != null) {
				...
				debug.drawLine(collision, collision.addMul(result.normal, 15, true), 0xaa0000);
				collision.dispose();
		...
		output = space.bodiesInCircle(Vec2.weak(mouseX, mouseY), /*radius*/ 100, /*strict-containment*/ false, /*filter*/ null, output);
		for (b in output) {
		...
		output = space.bodiesInCircle(Vec2.weak(mouseX, mouseY), /*radius*/ 100, /*strict-containment*/ true, /*filter*/ null, output);
		for (b in output) {
		...
		// With intersection only check.
		output = space.bodiesInAABB(sampleAABB, /*strict-containment*/ false, /*strict-check*/ true, /*filter*/ null, output);
		for (b in output) {
		...
		// With strict containment check.
		output = space.bodiesInAABB(sampleAABB, /*strict-containment*/ true, /*strict-check*/ true, /*filter*/ null, output);
		for (b in output) {
		...
		// With intersection only check.
		output = space.bodiesInShape(sampleShape, /*strict-containment*/ false, /*filter*/ null, output);
		for (b in output) {
		...
		// With strict containment check.
		output = space.bodiesInShape(sampleShape, /*strict-containment*/ true, /*filter*/ null, output);
		for (b in output) {
