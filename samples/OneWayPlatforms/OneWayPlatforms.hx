...
class OneWayPlatforms extends Template {
	...
	override function init() {
		...
		space.listeners.add(new PreListener(InteractionType.COLLISION, oneWayType, CbType.ANY_BODY, oneWayHandler, /*precedence*/ 0, /*pure*/
			true));
		...
		space.listeners.add(new InteractionListener(CbEvent.BEGIN, InteractionType.SENSOR, CbType.ANY_BODY, teleporterType, teleporterHandler));
	...
	function teleporterHandler(cb:InteractionCallback) {
		...
		if (object.type == BodyType.KINEMATIC)
			return;
