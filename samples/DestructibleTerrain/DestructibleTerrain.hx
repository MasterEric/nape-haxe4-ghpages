...
class DestructibleTerrain extends Template {
	...
	function createObject(pos:Vec2) {
		...
		if (Math.random() < 0.333) {
			...
			body.shapes.add(new Polygon(Polygon.regular(/*radiusX*/ 10 + Math.random() * 20, /*radiusY*/ 10 + Math.random() * 20,
	...
	public function invalidate(region:AABB, space:Space) {
		...
		for (y in y0...(y1 + 1)) {
			for (x in x0...(x1 + 1)) {
				...
				var polys = MarchingSquares.run(iso, isoBounds, isoGranularity, isoQuality);
				if (polys.empty())
					continue;
