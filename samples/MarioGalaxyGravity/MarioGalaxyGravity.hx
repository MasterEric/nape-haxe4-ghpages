...
class MarioGalaxyGravity extends Template {
	...
	override function init() {
		...
		var polys = MarchingSquares.run(starIso.iso, new AABB(0, 0, w, h), new Vec2(5, 5));
		for (poly in polys) {
	...
	function planetaryGravity(planet:Body, deltaTime:Float) {
		...
		for (body in space.liveBodies) {
			...
			body.applyImpulse(/*impulse*/ force.muleq(deltaTime), /*position*/
				null, // implies body.position
				/*sleepable*/
				true);
		}
