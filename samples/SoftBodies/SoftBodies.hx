...
class SoftBodies extends Template {
	...
	override function init() {
		...
		for (y in 4...7) {
			for (x in -2...3) {
				var body = polygonalBody(Vec2.get(w / 2 + x * 60, h - (y + 0.5) * 60), /*thickness*/ 10, /*discretisation*/ 15, /*frequency*/ 30, /*damping*/
					10, poly);
	...
	function polygonalBody(position:Vec2, thickness:Float, discretisation:Float, frequency:Float, damping:Float, poly:GeomPoly) {
